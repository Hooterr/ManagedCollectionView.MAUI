...
public class ManagedCollectionViewControl : Grid
{
    ...
    private void PgrOnPanUpdated(object? sender, PanUpdatedEventArgs e)
    {
        ...
        else
        {
            _previousPanY = 0;
            var a = 100d;
            var velocity = _velocityTracker.ComputeVelocityY(100);
            var t = (uint)(Math.Abs(velocity) / 0.7d);
            Debug.WriteLine($"Fling with velocity: {velocity:N5}, calculated duration: {t:N0}ms");
            new Animation(ProcessScroll, velocity / 10, 0d, Easing.SinOut).Commit(this, "Scroll", length: t);
        }
    }
